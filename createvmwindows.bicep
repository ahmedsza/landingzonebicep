...
resource vmName_CustomScript 'Microsoft.Compute/virtualMachines/extensions@2017-03-30' = if (deployAgent) {
  parent: vm_small
  name: 'CustomScript'
  location: location
  properties: {
    ...
    settings: {
      fileUris: [
        uri(artifactsLocation, 'setupagent.ps1${artifactsLocationSasToken}')
      ]
      commandToExecute: 'powershell.exe -ExecutionPolicy Unrestricted -Command "& {./setupagent.ps1 ${vstsParameters}}"'
    }
  }
}
